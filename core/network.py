...
from core.backbones.wideresnet import *
...
#         self.init_param()
    ...
    def build_graph(self):
        # Conv layers
        if self.base_name is None or self.base_name=='resnet18':
            self.base = resnet18(pretrained=True, **{'num_classes':self.y_dim})
            self.h_dim=512
        elif self.base_name=='resnet34':
            self.base = resnet34(pretrained=True, **{'num_classes':self.y_dim})
        # Final mixture of logits layer
...
class MixtureLogitNetwork_wideresnet(nn.Module):
    def __init__(self,
                 name       = 'mln',        # name
                 h_dim      = 640,          # itermediate feature dimension
                 y_dim      = 10,           # output dimension
                 USE_BN     = True,         # whether to use batch-norm
                 k          = 5,            # number of mixtures
                 sig_min    = 1e-4,         # minimum sigma
        ...
        super(MixtureLogitNetwork_wideresnet,self).__init__()
        self.name       = name
        self.h_dim      = h_dim
        self.y_dim      = y_dim
        self.USE_BN     = USE_BN
...
#         self.init_param()
    ...
    def build_graph(self):
        self.base =  WideResNet(28, 10, 10, 0.3).to('cuda')
    ...
    def forward(self,x):
        bs = x.size(0)
        feat = self.base.feature_extract(x)
        feat = self.avgpool(feat)
        feat = feat.view(feat.size(0),-1)
        mln_out = self.mol(feat)
        return mln_out # mu:[N x K x D] / pi:[N x K] / sigma:[N x K x D]
...
class MixtureLogitNetwork_mlp(nn.Module):
    ...
    def build_graph(self):
        self.layers = []
        # Conv layers
        prev_h_dim = self.x_dim[0] * self.x_dim[1]*self.x_dim[2] # input channel 
        self.layers.append(nn.Flatten())
        for h_dim in self.h_dims:
            self.layers.append(
                nn.Linear(
